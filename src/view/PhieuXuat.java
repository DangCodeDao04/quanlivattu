...
public class PhieuXuat extends javax.swing.JInternalFrame {
    ...
    public PhieuXuat() throws SQLException, ClassNotFoundException {
    ...
    public void init() throws SQLException, ClassNotFoundException{
        ...
        ResultSet rs = phieuxuat.GetData();
        ResetTable();
        int i = 0;
        while(rs.next()){
            ...
            tbphieuxuat.setValueAt(rs.getString("TrangThai"), i, 5);         
            i++;
        }
        phieuxuat.Close();
    }
    ...
    private void initComponents() {
        ...
        jComboBoxLuaChon.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Mã phiếu xuất", "Mã khách hàng", "Tên khách hàng" }));
        ...
        tbphieuxuat.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã phiếu xuất", "Mã khách hàng", "Tên khách hàng", "Ngày tạo", "Tổng tiền", "Trạng thái"
            }
        ));
    ...
    private void btxoaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btxoaMouseClicked
        int n = tbphieuxuat.getSelectedRow();
        String maphieuxuat = tbphieuxuat.getValueAt(n, 0).toString();       
        try {
    ...
    private void tftimkiemKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tftimkiemKeyReleased
        ...
        try {
            PhieuXuat_md phieuxuat = new PhieuXuat_md();
            ResultSet rs = phieuxuat.Search(timkiem);
            ResetTable();
            int i = 0;
            while(rs.next()){
                tbphieuxuat.setValueAt(rs.getString("MaPX"), i, 0);
    ...
    private void btxemchitietMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btxemchitietMouseClicked
        ...
        if(n == -1) {
            ...
            String maphieuxuat = tbphieuxuat.getValueAt(n, 0).toString(); 
            try {
                new PhieuXuatDetail(maphieuxuat).setVisible(true);
            } catch (SQLException ex) {
    ...
    private void bttimkiemMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_bttimkiemMouseClicked
        ...
        if(!tugia.isEmpty() && !dengia.isEmpty()){
            ...
            PhieuXuat_md phieuxuat;
            try {
                phieuxuat = new PhieuXuat_md();
                ResultSet rs = phieuxuat.TimKiemTheoGia(tu, den);
                ResetTable();
                int i = 0;
                while(rs.next()){
                    tbphieuxuat.setValueAt(rs.getString("MaPX"), i, 0);
            ...
            try {
                ...
                PhieuXuat_md phieuxuat = new PhieuXuat_md();
                ResultSet rs = phieuxuat.TimKiemTheoNgay(ngaybatdau, ngayketthuc);
                ResetTable();
                int i = 0;
                while (rs.next()) {
                    tbphieuxuat.setValueAt(rs.getString("MaPX"), i, 0);
    ...
    private void btsuaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btsuaMouseClicked
        ...
        String maphieuxuat = tbphieuxuat.getValueAt(n, 0).toString();    
        try {
            new UpdatePhieuXuat(maphieuxuat).setVisible(true);
        } catch (SQLException ex) {
            Logger.getLogger(PhieuXuat.class.getName()).log(Level.SEVERE, null, ex);
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(PhieuXuat.class.getName()).log(Level.SEVERE, null, ex);
